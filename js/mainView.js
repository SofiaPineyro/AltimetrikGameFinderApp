...
const showMenu = function() {
    ...
    sideMenu.style.property = "transition: 0.5s;";
};

const closeMenu = function() {
    const sideMenu = document.querySelector("aside");
    sideMenu.classList.remove("show-flex");
    sideMenu.style.property = "transition: 0.5s;";
};
...
window.addEventListener("click", function(e) {
    ...
    !headerOptions.contains(e.target) &&
    sideMenu.classList.contains("show-flex")) {
        closeMenu();
    }
});
...
const rowView = function() {
    const cards = document.querySelectorAll(".main__card");
    const cardsContainer = document.querySelector(".main__cards");
    cardsContainer.classList.remove("view-one");
    cards.forEach((element) => {
        element.classList.remove("view-one");
    });
};
...
const columnView = function(){
    const cards = document.querySelectorAll(".main__card");
    const cardsContainer = document.querySelector(".main__cards");
    cardsContainer.classList.add("view-one");
    cards.forEach((element) => {
        element.classList.add("view-one");
    });
};

// Format the date given by the API 
...
// Add the icons of the corresponding platforms, by their id
const addPlatformIcon = function(platforms) {
    let platformsList = "";
    platforms.map((platform) => {
        switch(platform.platform.id) {
            ...
            case 6:
                ...
                break;
            case 7:
                platformsList += nintendo;
                break;
            case 8:
                platformsList += android;
                break;
            default: 
                break;
        }
...
// Gets game description using the game id
const getDescription = async function(id) {
    let descriptionContainer = document.getElementById(`description "{id}`);
    if(descriptionContainer) {
        try {
            let response = await fetch(
                `https://api.rawg.io/api/games/${id}?key=a3bd3ba900174071b69fb84b6704339a`
            );
            let details = await response.json();
            descriptionContainer.innerHTML = game.dateFormat.descriptionContainer;
        }
        catch(e){
            descriptionContainer.innerHTML = "Description not available.";
            console.log(e);
        }
    }
};

const loadCards = function(games) {
    const cardsContainer = document.querySelector(".main__cards");
    const heading = document.querySelector(".main__heading");
    const subheading = document.querySelector(".main__subheading");
            ...
            <button onclick="openCard(${game.id})" class="card-button">
                ...
                <div class="card__content">
                    <div class="card__content-info">
                        ...
                        <p class="info__description" id="description ${game.id}"></p>
                    </div>
                    <div class="card__content-buttons">
                        <div class="buttons__platforms">
                            ${addPlatformIcon(game.parent_platforms)}
                        </div>
                        <p>#${index +1}</p>
                        <div class="buttons__wishlist">
                            <svg width="7" height="7" viewBox="0 0 7 7" xmlns="http://www.w3.org/2000/svg">
                            <path d="M2.7832 4.41406H0.515625V3.13086H2.7832V0.845703H4.06641V3.13086H6.33398V4.41406H4.06641V6.66992H2.7832V4.41406Z"/>
                            </svg>
...
const goHome = async function() {
    ...
    try {
        let response = await fetch(
            `https://api.rawg.io/api/games?key=a3bd3ba900174071b69fb84b6704339a&page_size=21`
        );
        let games = await response.json();
        cardsContainer.innerHTML = "";
        heading.innerHTML = "New and trending";
        subheading.innerHTML = "Based on player counts and release date";
        loadCards(games);
    } 
    catch(e){
        cardsContainer.innerHTML = "";
...
const getGameImages = async function (slug) {
    const imagesContainer = document.querySelector(".modal__images");
    let e = document.createElement("div");
    e.classList.add("loading");
    imagesContainer.appendChild(e);
    try {
        console.log(slug);
        let response = await fetch(
            `https://api.rawg.io/api/games/${slug}/screenshots?key=a3bd3ba900174071b69fb84b6704339a`
        );
        ...
        for (let i = 0; i < 5; i++) {
            if(i === 0) {
                ...
                `<div class="modal__image-container">
                    <img class="img-big" src=${
                    screenshots.results[i].image || "...\img\images\not-found.jpeg"
                    } />
                </div>`
                );
            }
...
const openCard = async function (id) {
    const cardsContainer = document.querySelector(".main__cards");
    ...
    modal.insertAdjacentHTML("beforeend", 
        `<img class = "modal__image" src=${game.background_image} />
            ...
            <div class="modal__wrap">
                ...
                <div class="modal__details">
                    <div class="modal__details-text">
                        <p>Platforms</p>
                        <a class="modal__link">${game.parent_platforms.map((platforms) => {
                            return " " + platforms.platform.name;    
                        })}</a>
                    </div>
                    <div class="modal__details-text">
                        <p>Genre</p>
                        <a class="modal__link">${game.genres?
                    ...
                    <div class="modal__details-text">
                        <p>Developer</p>
                        <a class="modal__link">${game.developers ? game.developers[0].name : "None"}</a>
                    </div>
                    <div class="modal__details-text">
                        <p>Pubisher</p>
                        <a class="modal__link">${game.publishers ? game.publishers.map((publisher) => {
                    ...
                    <div class="modal__details-text">
                        <p>Website</p>
                        <a class="modal__link">${game.website ? game.website : "None"}</a>
                    </div>
                </div>
